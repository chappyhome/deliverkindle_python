...
def callback_on_Books_post_delete(sender,**kwargs):
    ...
    #reset series
    series_obj = BooksSeriesLink.objects.using('slave').filter(book=book_id)
    if series_obj:
        res = BooksSeriesLink.objects.using('slave').filter(series=series_obj[0].series)
...
def callback_on_Books_post_save(sender,**kwargs):
    ...
    #books_click = BooksplusIdClick.objects.using('slave').get(id=book_id)
...
def callback_on_Comments_post_save(sender,**kwargs):
    from django.forms.models import model_to_dict
    import cPickle

    book_id = kwargs['instance'].book

    books_object = BooksAdd.objects.using('slave').get(id=book_id)
    r.hset(CALIBRE_ALL_BOOKS_OBJ_HASH, book_id, cPickle.dumps(books_object))
    r.hset(CALIBRE_ALL_BOOKS_HASH, book_id, json.dumps(model_to_dict(books_object),cls=ComplexEncoder))

    data = json.dumps(model_to_dict(books_object),cls=ComplexEncoder)
    es.index("readream", "books", body=data, docid=book_id)

signals.post_delete.connect(callback_on_Books_post_delete, sender=Books)
signals.post_save.connect(callback_on_Books_post_save, sender=Books)
signals.post_save.connect(callback_on_Comments_post_save, sender=Comments)
